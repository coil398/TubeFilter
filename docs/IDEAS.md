...
|---|---|---|---|
| **Channel allow/blocklist** — always show favourite small creators / always hide specific channels, regardless of view count | High | M | **done** |
| **Title keyword / regex filter** — hide videos whose title matches user words (clickbait, spoilers) | High | M | **done** |
...
| **More surfaces** — Subscriptions feed, watch-page sidebar recommendations, channel pages | High | M | **done** |
| **Selector robustness + self-diagnostics** — resist YouTube DOM changes | Med | M | **done** |
...
| Idea | Value | Effort | Status |
|---|---|---|---|
| Downscale extension icons to real 16/48/128 PNGs (were 1024px JPEGs) | Low | S | **done** |
...
| Translate the 8 non-English READMEs' "popup UI languages" prose to say 9 (currently still say ja/en) | Low | S | **done** |
